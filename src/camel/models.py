...
from camel.ext.verification_integration import verification_enabled
...
_supported_model_names = {
    ...
    "phi": "Phi-2 local model",
    "phi3.5": "Phi-3.5 local model",
    "wizardlm2": "Wizardlm2 local model",
    "mistral-nemo": "Mistral Nemo local model",
    "qwen2.5": "Qwen 7b local model",
    "qwen2.5-small": "Qwen 1.5b local model",
    "xLAM2": "Salesforce local model",
} | _oai_thinking_models_with_effort
...
def make_tools_pipeline(
    ...
    if "google" in model:
        ...
        from camel.token_tracker import TrackedAsyncAnthropic
        client = TrackedAsyncAnthropic(api_key=os.getenv("ANTHROPIC_API_KEY"))
        ...
        # Local model support (e.g., Ollama, vLLM, llama.cpp)
    ...
    if use_original:
        ...
        secpol_engine = engine if enable_secpol else ADNoSecurityPolicyEngine
        tools_pipeline = agent_pipeline.AgentPipeline(
            [
