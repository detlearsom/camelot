...
_REPAIR_SYSTEM_PROMPT_BASE = """\
Repair the previous plan. Return only complete repaired code.

Rules:
- Fix only the violation report; preserve unrelated behavior, required side effects, and required payloads.
- Use only available tool calls and CaMeL built-ins already allowed by the original prompt.
- No imports, eval, exec, while loops, generators, def/lambda, exception handling, or side-effecting methods.
- `break` is allowed only inside `for` loops; `continue` is unsupported.
- Prefer structured values or `query_ai_assistant` over regex parsing.
...
def function_to_signature(function: "functions_runtime.Function") -> str:
    """Returns only the def line — no docstring, no body."""
    name = function.name
    parameters = _get_function_parameters(function.parameters.model_fields)
    return f"def {name}({parameters}) -> {_get_type_string(function.return_type)}: ..."


def make_repair_system_prompt(functions: "Iterable[functions_runtime.Function]") -> str:
    """Minimal system prompt for CTL repair calls — signatures only, no docstrings."""
    sigs = "\n".join(function_to_signature(f) for f in functions)
    return _REPAIR_SYSTEM_PROMPT_BASE.format(tool_signatures=sigs)
...
- You may use `break` inside `for` loops. You can't use `continue` statements.
