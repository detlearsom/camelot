...
class AgentDojoSecurityPolicyEngine(SecurityPolicyEngine, Generic[_E]):
    ...
    def get_allowed_tools(self) -> list[str]:
        """Returns the list of tool names that have security policies defined.

        This includes both tools with explicit policies and tools marked as having no side effects.
        """
        policy_tools = [tool_name for tool_name, _ in self.policies]
        return policy_tools + list(self.no_side_effect_tools)
