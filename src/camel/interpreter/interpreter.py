...
@dataclasses.dataclass(frozen=True)
class _BreakSignal:
    """Internal sentinel returned by ast.Break; consumed by _eval_for."""
    node: ast.Break


def _is_break_signal(v: object) -> bool:
    return isinstance(v, _BreakSignal)
...
def _eval_module(
    ...
    val_res, namespace, tool_calls_chain, dependencies = _eval_stmt_list(
        node.body, namespace, tool_calls_chain, dependencies, eval_args
    )
    if isinstance(val_res, result.Ok) and isinstance(val_res.value, _BreakSignal):
        break_signal = val_res.value
        return EvalResult(
            result.Error(CaMeLException(SyntaxError("'break' outside loop"), (break_signal.node,), ())),
            namespace,
            tool_calls_chain,
            dependencies,
        )
...
def _eval_if(
    ...
    if _is_break_signal(body_res.value):
        return EvalResult(body_res, namespace, tool_calls_chain, dependencies)
...
def _eval_for(
    ...
    for elt in iterable.iterate_python():
        ...
        if _is_break_signal(final_val_res.value):
            broke = True
            break
    ...
    if node.orelse and not broke:
        ...
        if isinstance(final_val_res, result.Error):
            return EvalResult(final_val_res, namespace, tool_calls_chain, dependencies)
        if isinstance(final_val_res.value, _BreakSignal):
            return EvalResult(
                result.Error(CaMeLException(SyntaxError("'break' outside loop"), (final_val_res.value.node,), ())),
                namespace,
                tool_calls_chain,
                dependencies,
            )
